...
const CategoryUpdatePage = () => {
    ...
    try {
      ...
      if (res.status && res.data) {
        ...
        if (data.subcategories && Array.isArray(data.subcategories)) {
          existingSubs = data.subcategories.map((sub: any) => ({
            id: sub.id,
            name: sub.name
  ...
  useEffect(() => {
    if (!isAuthChecking && categoryId && token) {
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (finalSubcategories.length > 0) {
        putData.subcategories = finalSubcategories.map(sub => ({ 
          name: sub.name 
        }));
      }

      console.log('Sending update data:', putData);
                ...
                <Form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Form.Text className="text-muted">
                      Add subcategories one by one. You can also type and click "Update Category" - it will be automatically added.
                    </Form.Text>
                  </Form.Group>
                  ...
                  {subcategoryInput.trim() && (
                    <div className="alert alert-info mb-3">
                      <small>
                        <strong>Note:</strong> "{subcategoryInput}" will be automatically added when you click "Update Category".
                      </small>
                    </div>
                  )}
